 ...
 * 该对象会将标记成软删除的doc 也过滤掉   这样在searcher中 就无法查询到了
 */
public final class SoftDeletesDirectoryReaderWrapper extends FilterDirectoryReader {
    ...
    private static class SoftDeletesSubReaderWrapper extends SubReaderWrapper {
        ...
        public LeafReader wrap(LeafReader reader) {
            ...
            try {
                // 将reader 包装后 确保searcher能过滤被标记成软删除的doc
    ...
    static LeafReader wrap(LeafReader reader, String field) throws IOException {
        ...
        // 如果reader.getLiveDocs()一开始就包含了 软删除 那么numSoftDeletes 的值就为0  如果一开始仅包含硬删除
        // 在PendingSoftDeletes.applySoftDeletes(iterator, bits)后 就会加上软删除的数量  numDeletes 总是等于 soft+hard
