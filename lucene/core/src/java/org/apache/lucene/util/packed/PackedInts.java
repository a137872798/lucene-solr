 ...
 *
 * @lucene.internal 用于压缩 无符号的long值
 * 整齐的整数
 * 也就是每个值 都使用相同的bits
 */
public class PackedInts {
    ...
    public enum Format {
         ...
         * 因为  非整齐模式已经被弃用了  可以认为该方法就是通过整齐模式来触发的
         */
        public long byteCount(int packedIntsVersion, int valueCount, int bitsPerValue) {
         ...
         * values of size <code>bitsPerValue</code>.
         * 判断这么多位 一共需要多少long来保存
         */
        public int longCount(int packedIntsVersion, int valueCount, int bitsPerValue) {
            assert bitsPerValue >= 0 && bitsPerValue <= 64 : bitsPerValue;
            // 就是 (valueCount * bitsPerValue)/8  计算总计要消耗多少 byte
         ...
         * 默认情况下不使用额外的内存存储数据 这样就是按位存   优点是节省空间 缺点是读取相对较慢
         */
        public float overheadPerValue(int bitsPerValue) {
            assert isSupported(bitsPerValue);
            return 0f;
     ...
     * 该对象包含了使用的格式 以及 每个值消耗多少bit    就是一个简单的bean 对象
     */
     ...
     *
     * @lucene.internal 该对象 仅负责从RAM 中读取数据  注意读取的数据类型是 long
     */
     ...
     * 代表一个可变的数据块   reader本身定义成针对该数据块的读取功能 这里就是加入了写的逻辑
     * @lucene.internal
     */
     ...
     * A write-once Writer.
     *
     * @lucene.internal 用于写入数据的 对象 只能写入一次
     */
     ...
     *
     * @param valueCount              the number of elements   总计要写入多少数据
     * @param bitsPerValue            the number of bits available for any given value  每个值使用的bit数
     * @param acceptableOverheadRatio an acceptable overhead   使用的额外开销
     *                                ratio per value
     ...
     * @lucene.internal 估算某个long值需要至少多少bit来表达
     */
    ...
    public static int unsignedBitsRequired(long bits) {
        // Long.numberOfLeadingZeros 代表该值按二进制前面有多少连续的0
     ...
     * bytes.
     * @param mem 默认情况为 1024 byte  代表暂用128个long[]
     */
