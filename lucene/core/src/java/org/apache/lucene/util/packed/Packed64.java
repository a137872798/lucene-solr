 ...
 * 该对象尽可能的用位来表示写入的数据  比如写入一个long 值为1  那么实际上只会写入1位   这样内存开销就变成了 1/64
 */
class Packed64 extends PackedInts.MutableImpl {
     ...
     * Packet64 在创建的时候 就知道要占用多少空间  该数组就是用于存储数据的容器
     */
     ...
     *
     * @param valueCount   the number of elements.     代表一共要写入多少数据
     * @param bitsPerValue the number of bits available for any given value.   每个数据占用多少位
     */
    public Packed64(int valueCount, int bitsPerValue) {
        super(valueCount, bitsPerValue);
        // 默认采用紧凑的方式
        final PackedInts.Format format = PackedInts.Format.PACKED;
        // 简单理解为 (valueCount*bitsPerValue)/64
...
//    assert (((long)index * bitsPerValue) & MOD_MASK) == 0;
     ...
     * 代表往该数组中填充数据
     *
     * @param index  就是 arr的下标
     * @param arr   存储待写入数据的数组
     * @param off
     * @param len
     * @return
     */
    ...
    public int set(int index, long[] arr, int off, int len) {
        ...
        // go to the next block where the value does not span across two blocks
        // 如果余数为0 代表刚好从某个block的起点开始 否则就是往一个已经包含部分数据的block继续填充数据
        // index % encoder.longValueCount() 应该可以换算成  (index*bitsPerValue % encoder.longValueCount()*bitsPerValue)/bitsPerValue
        // 而后面又是64的倍数 能够被整除 代表刚好换成了一个新的block
        ...
        // 按照整块来填充   这里起点一定是某个block的开始
        // bulk set
        assert index % encoder.longValueCount() == 0;
        // 每个值实际写入只占 bitsPerValue  所以可以推算出 block下标
        int blockIndex = (int) (((long) index * bitsPerValue) >>> BLOCK_BITS);
        assert (((long) index * bitsPerValue) & MOD_MASK) == 0;
        final int iterations = len / encoder.longValueCount();
        // 在 encode中 从arr 读取的长度是 (iterations*longValueCount)
