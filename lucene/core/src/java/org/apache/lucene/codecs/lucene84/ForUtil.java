...
final class ForUtil {
     ...
     *
     * @param longs        待写入的数据
     * @param bitsPerValue 最大值占用的位数
     *                     将数据写入到 out中
     */
    void encode(long[] longs, int bitsPerValue, DataOutput out) throws IOException {
        final int nextPrimitive;
        final int numLongs;
        // 因为每个槽实际上只使用8位 不需要一个long值 所以进行压缩
        if (bitsPerValue <= 8) {
            nextPrimitive = 8;
            // 这个就对应压缩后的结果数量
            numLongs = BLOCK_SIZE / 8;
            // 将128个long 压缩成 16个值   (每个long取8位 总计8个long 拼接成新的long)
            collapse8(longs);
        } else if (bitsPerValue <= 16) {
            nextPrimitive = 16;
            // 每个long 取16位 也就是一个long 需要原本4个long值
        ...
        int idx = 0;
        // 实际存储占用的位数 与 读取位数的偏差值
        int shift = nextPrimitive - bitsPerValue;
        // tmp 数组大小为  block/2   为什么循环次数不是 2*nextPrimitive    这里感觉是要进一步的压缩
        for (int i = 0; i < numLongsPerShift; ++i) {
            // 读取出来的longs[idx++] 最高位可能会有空位  因为 nextPrimitive != bitsPerValue   通过 << shift后 确保此时所有高位都被填满
        ...
        // TODO 下面的不看了 推测就是尽可能不浪费任何一位  当 bitsPerValue 小于8的倍数时 也能填充空位

        // 代表每个long最少还有多少位空缺   对应  shift = shift - bitsPerValue
